...
function toMarkdownLine(sentenceHtml: string): string {
  ...
  return parts
    .map((chunk, i) =>
      i % 2 === 1
        ? `<span class="text-slate-500">**</span><span class="font-bold text-white">${escapeHtml(chunk)}</span><span class="text-slate-500">**</span>`
        : escapeHtml(chunk),
    )
    .join('')
}
...
function markdownLink(text: string, url: string) {
  return (
    ...
    `<span class="text-slate-500">](</span><span class="text-orange-300">${escapeHtml(url)}</span><span class="text-slate-500">)</span>`
  )
}
...
const bioLines = computed(() => {
  ...
  return [
    '<span class="text-slate-500"># </span><span class="font-bold text-white">About me</span>',
    '',
    ...sentences,
    '',
    ...socialLines,
...
<template>
  <div>
    ...
    <button
      type="button"
      class="font-jetbrains text-sm mt-2 disabled:opacity-40 disabled:cursor-not-allowed"
      :disabled="isDisabled"
      @click="downloadResume()"
    >
      <span class="text-slate-500">[</span><span class="text-blue-400 hover:underline">Download CV</span
      ><span class="text-slate-500">](</span><span class="text-orange-300">./resume.pdf</span><span class="text-slate-500">)</span>
    </button>
  </div>
</template>
